      ...
      of: [
        {
          ...
          fields: [
            ...
            },
            {
              ...
              type: 'string',
              description: 'ID de lecture Mux (depuis dashboard.mux.com → asset → Playback ID)',
            },
            {
              name: 'url',
              title: 'URL (YouTube, Vimeo, ou lien direct MP4)',
              type: 'url',
              description: 'Optionnel si Mux Playback ID est renseigné',
            },
            {
              name: 'aspectRatio',
              title: 'Format',
          ...
          preview: {
            select: { title: 'title', muxId: 'muxPlaybackId', url: 'url' },
            prepare({ title, muxId, url }: { title?: string; muxId?: string; url?: string }) {
              return {
                title: title || 'Sans titre',
                subtitle: muxId ? `Mux: ${muxId}` : url || 'Aucune source',
              }
            },
          },
        },
